...
// platform/web/src/lib/components.ts — same "copy the static data" approach as
// the archetype list. Keep in sync with the web TS copy and the Android Kotlin
// copies when adding a component.
//
// A client's enabled set lives in clients.enabled_components; the seller app
// reads it via ComponentsService (GET /api/seller/components) into
// AuthViewModel.enabledComponents. Gate UI with:
//   authViewModel.enabledComponents.contains(ComponentKeys.inquiryQuoteForm)
enum ComponentKeys {
