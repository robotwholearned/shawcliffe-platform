  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-md mx-auto space-y-6 pb-24">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          type="submit"
          disabled={submitting || !clientId || selectedItems.length === 0}
        ...
        >
          {submitting ? 'Placing order…' : `Reserve ${selectedItems.reduce((s, i) => s + i.quantity, 0)} item${selectedItems.reduce((s, i) => s + i.quantity, 0) === 1 ? '' : 's'}`}
        </button>
      </form>
    </div>
  )
